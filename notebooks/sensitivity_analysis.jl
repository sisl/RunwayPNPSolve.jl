...
In this notebook, we aim to investigate different possible represetations.
...
### The setup
#### Aircraft pose
For simplicity, we start by assuming a pose at the extreme of the service volume.
In particular, we choose maximum distance, minimum elevation, and assume no crosstrack error for now.
Therefore we set

$$\vec{p} = \begin{bmatrix} 6000\mathit{m} \\ 0\mathit{m} \\ 126\mathit{m} \end{bmatrix}.$$

Notice that the elevation is derived from the minimum vertical angle of $1.2\degree$, with $z/x = \tan{\alpha}$.
"""
...
md"""
#### Runway specs
For the runway, we consider the SFO runway, which has a length of about $3500\mathit{m}$ and a width of about $61\mathit{m}$.
Since the coordinate system has x in alongtrack and y in crosstrack direction, we denote the above measurements as
$\Delta x = 3500\mathit{m}$ and $\Delta y = 61\mathit{m}$.
Note however that for these calculations, we set the origin into the airplane, which make the equations easier.
"""
...
md"""
#### Camera
Finally, we consider the camera labeled "MDC selection" (see [here](https://docs.google.com/spreadsheets/d/1r2neGh5YUa2e5Ufr7xOfkrC9kr5bqifN5rn2pktkGS0/edit#gid=760597346)), which has
a focal length of $25\mathit{mm}$ and a pixel size of $0.00345\mathit{mm}$.
"""

# ╔═╡ acbf8686-9e45-406b-bb55-1ff5b53c2316
f, pxl_size = 35mm, 0.00345mm

# ╔═╡ 354d7de0-d334-44bc-9106-96d6a30dc17e
md"""
### Approach 1: Runway length estimation
In the following we assume to estimate the runway length in pixel space and compute the alongtrack distance by referencing the true runway length.

Let us now denote $\Delta z\prime$ as a quantity in pixel space.
Then we can compute
...
Although not easy, we can solve this equation for $x$ and get

$$x(\Delta z\prime) = \sqrt{\left(\frac{\Delta x}{2}\right)^2 + f H \frac{\Delta x}{\Delta z\prime}} - \frac{\Delta x}{2}$$

and the derivative

$$\frac{\partial x}{\partial (\Delta z\prime)} = - \frac{1}{2} f H \frac{\frac{\Delta x}{(\Delta z\prime)^2}}{\sqrt{\left(\frac{\Delta x}{2}\right)^2 + f H \frac{\Delta x}{\Delta z\prime}}}.$$

We can use the derivative to compute the sensitivity, i.e. by relating $\epsilon_x / \epsilon_{\Delta z\prime} \approx \partial x / \partial (\Delta z\prime)$.

Notice how the second equation does not depend on $x$ directly (!); however, as the distance to the runway gets larger, we have to expect larger errors for $\Delta z$.
...
# ╔═╡ 50d1e309-06d6-4264-a03e-82cc75dfd735
